...
function my_portfolio_scripts() {
	...
	wp_enqueue_style( 'my-portfolio-all-min-css', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css', '', true );
	wp_enqueue_style( 'my-portfolio-googleapis-css', 'https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;600&display=swap', '', true );
	wp_enqueue_style( 'my-portfolio-custom-css', get_template_directory_uri() . '/css/style.css', '', true );

	wp_enqueue_script( 'my-portfolio-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

	wp_enqueue_script( 'my-portfolio-external-js', get_template_directory_uri() . '/js/main.js', array(), _S_VERSION, true );
...
/* Project URL Meta Field */
function add_project_url_meta() {
    add_meta_box(
        'project_url',
        'Project URL',
...
add_action('add_meta_boxes', 'add_project_url_meta');

function project_url_callback($post) {
    $value = get_post_meta($post->ID, 'project_url', true);
    ?>
    <input type="url" name="project_url" value="<?php echo esc_attr($value); ?>" style="width:100%;" />
    <?php
}
...
add_action('save_post', 'save_project_url');
